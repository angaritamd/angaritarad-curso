...
function ChatCard({ card }) {
    ...
    return (
      <div style={{ background: 'var(--canvas-card)', borderRadius: 16, padding: 24, border: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', gap: 16, height: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--muted)' }}>ANGARITARAD-AI</span>
        </div>
  ...
  return (
    <div style={{ background: 'var(--canvas-card)', borderRadius: 16, padding: 24, border: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, paddingBottom: 14, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--aurora-mint)' }} />
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--muted)' }}>ANGARITARAD-AI · WHATSAPP</span>
      </div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 14 }}>
        <div style={{ background: 'rgba(255,255,255,0.1)', color: 'var(--body)', borderRadius: '12px 12px 4px 12px', padding: '9px 13px', maxWidth: '80%', fontSize: 13, lineHeight: 1.5 }}>
          {card.user}
        </div>
      </div>
      <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px 12px 12px 4px', padding: '12px 14px', flex: 1 }}>
        {card.lines.map((line, i) => (
          <div key={i} style={{ display: 'flex', gap: 6, marginBottom: i < card.lines.length - 1 ? 6 : 0 }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--primary)', flexShrink: 0 }}>{line.tag}</span>
            {line.text && <span style={{ fontSize: 13, color: 'var(--body)', lineHeight: 1.5 }}>{line.text}</span>}
          </div>
        ))}
      </div>
      <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--body)' }}>{card.label}</span>
      </div>
    </div>
  );
}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {examples.map((ex, i) => (
              <button key={ex.id} onClick={() => setActive(i)} style={{
                ...
                fontSize: 14, fontFamily: 'var(--font-sans)', transition: 'all 0.15s',
              }}>
                {ex.label}
              </button>
            ))}
